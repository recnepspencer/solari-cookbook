...
export const INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION = "read_recovery_projection" as const
...
export const INTERFACE_COMPILER_WORTH_REGISTER_VERIFICATION_EVIDENCE_OPERATION = "register_verification_evidence" as const
...
export type ReplayRecoveryOperation =
  ...
  | typeof INTERFACE_COMPILER_WORTH_REGISTER_VERIFICATION_EVIDENCE_OPERATION
...
export type HostResponse =
  ...
  | { readonly outcome: "lifecycle_not_pending"; readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: typeof INTERFACE_COMPILER_WORTH_ADMIT_EXECUTION_OPERATION; readonly execution_id: string; readonly current_lifecycle: string }
  ...
  | { readonly outcome: "execution_denied"; readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: typeof INTERFACE_COMPILER_WORTH_ADMIT_EXECUTION_OPERATION; readonly execution_id: string; readonly stage: HostStartDenialStage; readonly kind: string; readonly message: string }
  | { readonly outcome: "execution_denied"; readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: typeof INTERFACE_COMPILER_WORTH_COMPLETE_EXECUTION_OPERATION; readonly execution_id: string; readonly stage: HostStartDenialStage; readonly kind: string; readonly message: string }
  ...
  | { readonly outcome: "recovery_projection_found"; readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: typeof INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION; readonly capability: HostCapabilityProjection; readonly replay: HostActiveReplayProjection; readonly capability_evidence: HostEvidence; readonly replay_evidence: HostEvidence }
  | { readonly outcome: "compiled_read_not_found"; readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: ProjectionReadOperation; readonly capability_id: string }
  | { readonly outcome: "compiled_read_denied"; readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: ProjectionReadOperation; readonly capability_id: string; readonly stage: HostDenialStage; readonly kind: string; readonly message: string }
...
type ProjectionReadOperation = CompiledReadOperation | typeof INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION
...
export type HostRequest =
  ...
  | { readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: typeof INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION; readonly capability_id: CapabilityId; readonly credential: string; readonly deadline_ms: number }
  ...
  | { readonly protocol: typeof INTERFACE_COMPILER_WORTH_PROTOCOL; readonly request_id: string; readonly operation: typeof INTERFACE_COMPILER_WORTH_REGISTER_VERIFICATION_EVIDENCE_OPERATION; readonly capability_id: CapabilityId; readonly replay_version_id: string; readonly expected_capability_revision: number; readonly expected_replay_revision: number; readonly verification_evidence: unknown; readonly credential: string; readonly deadline_ms: number }
...
export function parseHostResponse(line: string): HostResponse | undefined {
  ...
  if (value.outcome === "lifecycle_not_pending" && value.operation === INTERFACE_COMPILER_WORTH_ADMIT_EXECUTION_OPERATION && isNonEmptyText(value.execution_id) && isNonEmptyText(value.current_lifecycle)) return { outcome: "lifecycle_not_pending", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: INTERFACE_COMPILER_WORTH_ADMIT_EXECUTION_OPERATION, execution_id: value.execution_id, current_lifecycle: value.current_lifecycle }
  ...
  if (value.outcome === "execution_denied" && value.operation === INTERFACE_COMPILER_WORTH_ADMIT_EXECUTION_OPERATION && isNonEmptyText(value.execution_id) && isHostStartDenialStage(value.stage) && isNonEmptyText(value.kind) && typeof value.message === "string") return { outcome: "execution_denied", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: INTERFACE_COMPILER_WORTH_ADMIT_EXECUTION_OPERATION, execution_id: value.execution_id, stage: value.stage, kind: value.kind, message: value.message }
  if (value.outcome === "execution_denied" && value.operation === INTERFACE_COMPILER_WORTH_COMPLETE_EXECUTION_OPERATION && isNonEmptyText(value.execution_id) && isHostStartDenialStage(value.stage) && isNonEmptyText(value.kind) && typeof value.message === "string") return { outcome: "execution_denied", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: INTERFACE_COMPILER_WORTH_COMPLETE_EXECUTION_OPERATION, execution_id: value.execution_id, stage: value.stage, kind: value.kind, message: value.message }
  if (value.outcome === "execution_denied" && value.operation === INTERFACE_COMPILER_WORTH_PUBLISH_DOMAIN_EVENT_OPERATION && isNonEmptyText(value.execution_id) && isHostStartDenialStage(value.stage) && isNonEmptyText(value.kind) && typeof value.message === "string") return { outcome: "execution_denied", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: INTERFACE_COMPILER_WORTH_PUBLISH_DOMAIN_EVENT_OPERATION, execution_id: value.execution_id, stage: value.stage, kind: value.kind, message: value.message }
  ...
  if (value.outcome === "recovery_projection_found" && value.operation === INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION && isHostCapability(value.capability) && isHostReplay(value.replay) && isHostEvidence(value.capability_evidence) && isHostEvidence(value.replay_evidence)) return { outcome: "recovery_projection_found", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION, capability: value.capability, replay: value.replay, capability_evidence: value.capability_evidence, replay_evidence: value.replay_evidence }
  if (value.outcome === "compiled_read_not_found" && isProjectionReadOperation(value.operation) && isNonEmptyText(value.capability_id)) return { outcome: "compiled_read_not_found", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: value.operation, capability_id: value.capability_id }
  if (value.outcome === "compiled_read_denied" && isProjectionReadOperation(value.operation) && isNonEmptyText(value.capability_id) && isHostDenialStage(value.stage) && isNonEmptyText(value.kind) && typeof value.message === "string") return { outcome: "compiled_read_denied", protocol: INTERFACE_COMPILER_WORTH_PROTOCOL, request_id: value.request_id, operation: value.operation, capability_id: value.capability_id, stage: value.stage, kind: value.kind, message: value.message }
...
function isHostCapability(value: unknown): value is HostCapabilityProjection {
  if (!isRecord(value) || value.projection_kind !== "worth_capability" || !isNonEmptyText(value.id) || !isRevision(value.revision) || !isNonEmptyText(value.application_id) || typeof value.name !== "string" || typeof value.description !== "string" || !isRecord(value.input_schema) || !isRecord(value.output_schema) || !Array.isArray(value.preconditions) || !Array.isArray(value.postconditions) || !isRecord(value.publication) || !isNonEmptyText(value.status)) return false
  ...
  if (value.status === "degraded") return value.active_replay_version_id !== undefined && value.broken_replay_version_id !== undefined && value.candidate_replay_version_id === undefined && value.failure !== undefined
  if (value.status === "verifying") return value.active_replay_version_id !== undefined && value.broken_replay_version_id !== undefined && value.candidate_replay_version_id !== undefined && value.failure !== undefined
  return false
}
function isHostReplay(value: unknown): value is HostActiveReplayProjection {
  ...
  if (value.status === "broken") return value.failure !== undefined && value.broken_at !== undefined
  return false
}
...
function isProjectionReadOperation(value: unknown): value is ProjectionReadOperation { return isCompiledReadOperation(value) || value === INTERFACE_COMPILER_WORTH_READ_RECOVERY_PROJECTION_OPERATION }
function isRecoveryOperation(value: unknown): value is ReplayRecoveryOperation { return value === INTERFACE_COMPILER_WORTH_DEGRADE_REPLAY_OPERATION || value === INTERFACE_COMPILER_WORTH_ACCEPT_REPLACEMENT_CANDIDATE_OPERATION || value === INTERFACE_COMPILER_WORTH_REGISTER_VERIFICATION_EVIDENCE_OPERATION || value === INTERFACE_COMPILER_WORTH_RECORD_REPLACEMENT_VERIFICATION_OPERATION || value === INTERFACE_COMPILER_WORTH_ACTIVATE_REPLACEMENT_OPERATION }
