...
const REPLACEMENT_ID: &str = "replay.demoblaze.samsung-galaxy-s6-to-order-boundary.v2";
const REPLACEMENT_EXPERIMENT_ID: &str = "experiment.recovery.samsung-galaxy-s6-to-order-boundary";
...
fn worth_owns_failure_degradation_exploration_verification_and_replacement_activation() {
    run_on_host_stack(|| {
        ...
        assert_eq!(degraded_capability.name, "SamsungGalaxyS6ToOrderBoundary");
